...
void VolumeMaterial::preprocess(){}


void VolumeMaterial::shade( rgb & result, const RenderContext & context, const ray & r, HitRecord & hit, int depth, double attenuation) const{
...
VolumeMaterial::VolumeMaterial(const std::string& data_fn,
 ...
 nearest(nearest_neighbor)
{
  ...
  if (nrrdLoad(nin, data_fn.c_tr(), NULL)) 
  {
    ...
    std::cerr << "VolumeMaterial: trouble reading " << data_fn << ": " <<  err << std::endl;
    free(err);
    exit(1);
  }
